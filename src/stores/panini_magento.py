...
def check_panini_magento(url: str, html: str | None = None) -> CheckResult:
    from src.stores.base import fetch_html_with_fallback

    page_html, note = fetch_html_with_fallback(url, html=html)
    if page_html is None:
        return CheckResult(status=StockStatus.UNKNOWN, reason=f"Failed to fetch page ({note})")
    result = magento_main_product_check(page_html, "panini.es")
